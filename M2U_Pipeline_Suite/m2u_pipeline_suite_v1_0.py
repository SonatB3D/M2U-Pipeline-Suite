# -*- coding: utf-8 -*-
"""
M2U Pipeline Suite v1.0
Professional Maya-to-Unreal preflight, prep, validation, report and FBX export tool.
...
Notes:
    ...
    - UCX_* transforms are ignored as base assets by default.
...
M2U_PIPELINE_BUILD_ID = "v1.0.8_regression_restore"
M2U_PIPELINE_WINDOW_TITLE = "M2U Pipeline Suite v1.0.8"
...
BASE_PRESET = {
    ...
    "expected_height_cm": 300.0,
    "expected_depth_cm": 20.0,
    ...
    "collision_requirement": "UCX optional",
    ...
    "lightmap_uv_names": "lightmap,Lightmap,UV1,uv1,map2",
    ...
    "write_unreal_import_script": True,
    "triangulate_export": False,
}


PRESETS = {}
PRESETS["Marketplace Static Mesh"] = _deepcopy_json(BASE_PRESET)
PRESETS["Marketplace Static Mesh"].update({
    "profile_name": "Marketplace Static Mesh",
    "collision_requirement": "UCX optional",
    "dimension_enabled": False,
    "grid_step_cm": 10.0,
    "max_polycount": 12000,
...
PRESETS["Modular Wall Kit"] = _deepcopy_json(BASE_PRESET)
PRESETS["Modular Wall Kit"].update({
    "profile_name": "Modular Wall Kit",
    "collision_requirement": "UCX required",
    "collision_severity": "blocking",
    "grid_step_cm": 50.0,
    "grid_check_size_multiple": True,
    "grid_severity": "blocking",
    "dimension_enabled": True,
    "expected_width_cm": 400.0,
    "expected_height_cm": 300.0,
    "expected_depth_cm": 20.0,
    "pivot_target": "Bottom Front Center",
    "pivot_severity": "blocking",
})

PRESETS["Furniture / Prop"] = _deepcopy_json(BASE_PRESET)
PRESETS["Furniture / Prop"].update({
    "profile_name": "Furniture / Prop",
    "collision_requirement": "UCX optional",
    "max_polycount": 20000,
    "pivot_target": "Bottom Center",
    "dimension_enabled": False,
...
PRESETS["Collision Strict"] = _deepcopy_json(BASE_PRESET)
PRESETS["Collision Strict"].update({
    "profile_name": "Collision Strict",
    "collision_requirement": "UCX required",
    "collision_severity": "blocking",
    "validate_ucx_meshes": True,
...
PRESETS["Nanite Candidate Check"] = _deepcopy_json(BASE_PRESET)
PRESETS["Nanite Candidate Check"].update({
    "profile_name": "Nanite Candidate Check",
...
COLLISION_REQUIREMENTS = ["Off", "UCX required", "UCX optional", "No custom collision allowed"]
...
class M2UPipelineEngine(object):
    ...
    def is_ucx_name(self, node_or_name):
        return self.clean_asset_name(node_or_name).startswith("UCX_")
    ...
    def _collision_compare_key(self, name):
        """Return a stable, case-insensitive key for UCX/name matching.

        Maya node names may arrive as long DAG paths and may include namespaces.
        UCX matching should compare the visible transform name only.
        """
        return self.clean_asset_name(name).strip().lower()

    def get_collision_target_candidates(self, target_name, required_prefix=None):
        """Build practical UCX target candidates for real production naming.

        Primary Unreal convention remains UCX_<RenderMeshName>. The helper also
        accepts common pre-rename variants, for example UCX_Chair_01 when the
        render mesh is SM_Chair_01 or ZM_Chair_01. This makes prefix migration
        safer without forcing the artist to manually rename collision meshes.
        """
        ...
        candidates = []
        prefix_list = ["SM_"]
        if required_prefix and required_prefix not in prefix_list:
            prefix_list.append(required_prefix)

        for item in [clean, self.strip_lod_suffix(clean)]:
            add_unique(candidates, item)
            for prefix in prefix_list:
                if prefix and item.upper().startswith(prefix.upper()):
                    add_unique(candidates, item[len(prefix):])
        return candidates

    def find_ucx_matches(self, target_name, accept_multiple_parts=True, required_prefix=None):
        ...
        base_patterns = ["ucx_{0}".format(k) for k in target_keys if k]

        for node in transforms:
            if not node or node in seen_paths:
                continue
            short_clean = self.clean_asset_name(node)
            short_key = self._collision_compare_key(node)
            if short_key in target_keys:
                continue
            if not short_key.startswith("ucx_"):
                continue
    ...
    def validate_ucx_mesh(self, transform, settings):
        ucx_name = self.clean_asset_name(transform)
        checks = {}
        ...
        polycount = self.get_polycount_for_transform(transform, include_descendants=True)
        max_poly = settings.get("max_polycount", 0)
        if max_poly < 0:
            checks["Polycount"] = self._make_check("Fail", "Max polycount cannot be negative")
        elif polycount <= max_poly:
            checks["Polycount"] = self._make_check("Pass", "{0} / {1}".format(polycount, max_poly))
        else:
            checks["Polycount"] = self._make_check("Fail", "{0} / {1}".format(polycount, max_poly))
        ...
        if not settings.get("zero_thickness_enabled") or not has_mesh:
            ...
            if thin_axes:
                checks["Zero Thickness"] = self._make_check(
                    "Fail",
                    "Near-zero axis: {0} | W/H/D: {1}".format(", ".join(thin_axes), self._format_float_list([width, height, depth]))
                )
            else:
    ...
    def validate_asset(self, transform, settings):
        ...
        missing_prefix = bool(settings.get("required_prefix")) and not asset_name.startswith(settings.get("required_prefix"))
        ...
        elif missing_prefix or needs_sanitize:
            reasons = []
            if missing_prefix:
                reasons.append("missing prefix {0}".format(settings.get("required_prefix")))
            if needs_sanitize:
        ...
        if not settings.get("collision_enabled") or settings.get("collision_requirement") == "Off":
            ...
            matches = self.find_ucx_matches(target_name, settings.get("accept_multiple_ucx_parts", True), required_prefix=settings.get("required_prefix")) if target_name else []
            ...
            if (matches and self._should_sync_ucx_names_on_rename(settings)):
                ...
                if preview_name != asset_name:
                    result["fix_plan"].append("Sync matching UCX names to renamed base asset")
            requirement = settings.get("collision_requirement")
            if requirement == "UCX required" and not matches:
                result["checks"]["Collision"] = self._make_check("Fail", "UCX collision required but no match found for {0}".format(target_name))
                self._add_issue(result, settings.get("collision_severity", "warning"), "Missing required UCX collision for target: {0}".format(target_name))
                result["manual_review"].append("Create UCX collision mesh or use Create Box UCX helper")
            elif requirement == "No custom collision allowed" and matches:
                result["checks"]["Collision"] = self._make_check("Fail", "Custom UCX collision is not allowed but {0} match(es) found".format(len(matches)))
                self._add_issue(result, settings.get("collision_severity", "warning"), "Custom collision is not allowed")
            else:
                result["checks"]["Collision"] = self._make_check("Pass", "{0} matching UCX mesh(es) found".format(len(matches)))
                ...
                for match in matches:
                    ...
                    if ucx_result.get("status") == "Fail":
                        self._add_issue(
                            ...
                            "UCX mesh validation failed for {0}: {1}".format(ucx_result.get("name"), ", ".join(ucx_result.get("failed_checks", [])))
                        )
    ...
    def _preview_renamed_asset_name(self, current_name, settings):
        ...
        prefix = settings.get("required_prefix") or ""
        if settings.get("fix_rename") and prefix and not clean.startswith(prefix):
            clean = prefix + clean
        return clean
    ...
    def _should_sync_ucx_names_on_rename(self, settings):
        return bool(
            ...
            and settings.get("collision_requirement") == "UCX required"
    ...
    def _expected_synced_ucx_name(self, ucx_name, old_base_name, new_base_name, settings):
        ucx_clean = self.clean_asset_name(ucx_name)
        ucx_key = self._collision_compare_key(ucx_clean)
        ...
        for candidate in candidates:
            ...
            pattern = "ucx_{0}".format(candidate_key)
            if ucx_key == pattern:
                return "UCX_{0}".format(new_base_name)
            if ucx_key.startswith(pattern + "_"):
                original_pattern_len = len("UCX_{0}".format(candidate))
                suffix = ucx_clean[original_pattern_len:]
                return "UCX_{0}{1}".format(new_base_name, suffix)
        return None
    ...
    def _sync_ucx_names_to_base(self, old_base_name, new_base_name, old_ucx_matches, settings):
        ...
        for match in old_ucx_matches:
            ...
            try:
                ...
                actions.append("Synced UCX name {0} -> {1}".format(old_ucx_name, self.clean_asset_name(renamed)))
            except Exception as exc:
                actions.append("UCX name sync failed for {0}: {1}".format(old_ucx_name, exc))
        return actions
    ...
    def reset_collision_preview_materials(self, result):
        """Restore UCX preview materials captured during this Maya session."""
        restored = 0
    ...
    def colorize_collision_preview(self, result):
        green = self._get_or_create_material("M2U_Collision_OK_Green", (0.2, 0.8, 0.2))
        red = self._get_or_create_material("M2U_Collision_Error_Red", (0.9, 0.15, 0.1))
        matches = result.get("collision", {}).get("matches", []) or []
        failed_names = set()
    ...
    def create_box_ucx_for_asset(self, transform, settings):
        ...
        base_name = "UCX_{0}_01".format(target_name)
        ucx_name = self._unique_name(base_name)
        width, height, depth = self.get_bbox_sizes(transform)
        center = self.get_bbox_center(transform)
        cube = cmds.polyCube(name=ucx_name, width=width, height=height, depth=depth)[0]
    ...
    def export_results(self, results, settings, export_folder):
        ...
        for result in results:
            ...
            # Always refresh UCX matching at export time. This prevents stale analysis
            # from exporting the base mesh alone after the user creates or renames UCX.
            collision_paths = []
            if settings.get("collision_enabled") and settings.get("collision_requirement") != "Off":
                ...
                if settings.get("collision_requirement") == "UCX required" and not collision_paths:
                    result["export"] = {
                        "status": "failed",
                        "message": "UCX required but no matching UCX collision was found at export time. Searched targets: {0}".format(
            ...
            if not self.should_export_result(result, settings):
                result["export"] = {"status": "skipped", "message": "Skipped by export rules", "included_ucx": len(collision_paths)}
            ...
            if success:
                result["export"] = {"status": "exported", "message": message, "path": export_path, "included_ucx": len(collision_paths)}
                export_summary["exported_assets"] += 1
            else:
                result["export"] = {"status": "failed", "message": message, "included_ucx": len(collision_paths)}
                export_summary["failed_exports"] += 1
        ...
        if settings.get("write_unreal_import_script"):
            try:
                export_summary["unreal_import_script"] = self.write_unreal_import_script(export_folder, results)
            except Exception:
                export_summary["report_errors"].append("unreal_import_script: {0}".format(traceback.format_exc()))
        return export_summary

    def write_json_report(self, export_folder, settings, results, export_summary):
        path = os.path.join(export_folder, "M2U_Pipeline_Report.json")
        payload = {
    ...
    def write_html_report(self, export_folder, settings, results, export_summary):
        path = os.path.join(export_folder, "M2U_Pipeline_Report.html")
        ...
        for r in results:
            ...
            coll_html = "".join(["<li>{0}</li>".format(html_escape(c.get("name", ""))) for c in collisions]) or "<li>No matching UCX mesh.</li>"
            ...
            <section class="asset-card">
                <div class="asset-head">
                    ...
                    <div class="score"><strong>__SCORE__</strong><span>M2U Score</span></div>
                </div>
                <div class="meta">
                    __STATUS__
                    __EXPORT_STATUS__
                    ...
                    <span>Included UCX: __INCLUDED_UCX__</span>
                </div>
                <h3>Issues</h3>
                <ul>__ISSUES__</ul>
                <h3>Fix Plan</h3>
                ...
                <h3>Collision Matches</h3>
                <ul>__COLLISION__</ul>
                <h3>Collision Target Candidates</h3>
            ...
            replacements = {
                ...
                "__INCLUDED_UCX__": html_escape(str(export.get("included_ucx", 0))),
...
<head>
<meta charset="utf-8">
<title>M2U Pipeline Report</title>
<style>
...
<body>
<header>
<h1>M2U Pipeline Suite v1.0.8 Report</h1>
<p>Professional Maya-to-Unreal static mesh preflight, cleanup, validation and export report.</p>
...
<main>__CARDS__</main>
<footer>M2U Pipeline Suite v1.0.8</footer>
</body>
</html>'''
        replacements = {
    ...
    def write_unreal_import_script(self, export_folder, results):
        path = os.path.join(export_folder, "M2U_Unreal_Import_Assets.py")
        exported = []
        for r in results:
            export = r.get("export", {}) or {}
            if export.get("status") == "exported" and export.get("path"):
                exported.append(export.get("path"))
        script = '''# Auto-generated by M2U Pipeline Suite v1.0.8
# Run inside Unreal Editor Python.
import os
import unreal

FBX_FILES = {fbx_files}
DESTINATION_PATH = "/Game/M2U_Imported"
...
def import_fbx_files():
    ...
    for fbx in FBX_FILES:
        if not os.path.exists(fbx):
            unreal.log_warning("Missing FBX: {{}}".format(fbx))
            continue
    ...
    if tasks:
        ...
        unreal.log("M2U import completed. Imported {{}} task(s) to {{}}".format(len(tasks), DESTINATION_PATH))
    else:
        unreal.log_warning("No valid FBX files found for M2U import.")

import_fbx_files()
'''.format(fbx_files=repr(exported))
        with io.open(path, "w", encoding="utf-8") as f:
            f.write(script)
        return path.replace("\\", "/")
...
class M2UPipelineSuiteWindow(QtWidgets.QDialog):
    WINDOW_OBJECT_NAME = "m2uPipelineSuiteWindow"
    ...
    def __init__(self, parent=None):
        ...
        self.last_results = []
        self.last_settings = _deepcopy_json(PRESETS["Marketplace Static Mesh"])
        self.current_export_summary = {}
        self._build_ui()
        self._apply_style()
        self.apply_preset("Marketplace Static Mesh")
    ...
    def _build_ui(self):
        ...
        title = QtWidgets.QLabel("M2U Pipeline Suite")
        title.setObjectName("M2UTitle")
        subtitle = QtWidgets.QLabel("Maya → Unreal preflight, safe cleanup, validation, collision, reports and FBX export.")
    ...
    def _build_wizard_tab(self):
        ...
        self.btn_reset_defaults.setToolTip("Restore the user-friendly Unreal-ready default settings and clear old displayed results. Scene objects are not modified.")
        ...
        self.write_unreal_cb = QtWidgets.QCheckBox("Write Unreal Import Script")
    ...
    def _build_rules_tab(self):
        ...
        self.ignore_ucx_cb = QtWidgets.QCheckBox("Ignore UCX_* As Base Asset")
        ...
        self.required_prefix_edit = QtWidgets.QLineEdit("SM_")
        ...
        advanced = self._card_group("Advanced Unreal Readiness")
    ...
    def _build_prep_tab(self):
        ...
        self.sync_ucx_names_cb = QtWidgets.QCheckBox("Sync UCX Names To Base Prefix When UCX Required")
        self.sync_ucx_names_cb.setToolTip("Example: Chair_01 + UCX_Chair_01 becomes SM_Chair_01 + UCX_SM_Chair_01 when the base prefix is SM_.")
        ...
        self.btn_create_ucx = QtWidgets.QPushButton("Create Simple Box UCX")
    ...
    def _build_collision_tab(self):
        tab = QtWidgets.QWidget()
        layout = QtWidgets.QVBoxLayout(tab)
        collision = self._card_group("Collision Rules")
        f = QtWidgets.QFormLayout(collision)
        self.collision_enabled_cb = QtWidgets.QCheckBox("Enable Collision Check")
        ...
        self.accept_multi_ucx_cb = QtWidgets.QCheckBox("Accept Multiple UCX Parts")
        self.validate_ucx_cb = QtWidgets.QCheckBox("Validate Matched UCX Meshes")
        ...
        f.addRow("UCX Mesh Validation Severity", self.ucx_validation_severity_combo)
        f.addRow("Collision Severity", self.collision_severity_combo)
        layout.addWidget(collision)

        actions = self._card_group("Collision Visualizer")
        gl = QtWidgets.QGridLayout(actions)
        self.btn_select_ucx = QtWidgets.QPushButton("Select Asset + Matching UCX")
        self.btn_isolate_ucx = QtWidgets.QPushButton("Isolate Asset + UCX")
        self.btn_color_ucx = QtWidgets.QPushButton("Colorize UCX Preview")
        self.btn_reset_ucx_materials = QtWidgets.QPushButton("Reset UCX Preview Materials")
        ...
        layout.addStretch(1)
        self.tabs.addTab(tab, "Collision")
    ...
    def _build_help_tab(self):
        ...
        help_text.setPlainText(
            ...
            "4. Review fix plan, warnings and blocked issues.\n"
            "5. Apply Safe Fixes if desired.\n"
            "6. Revalidate.\n"
            "7. Export FBX + JSON/HTML report + Unreal import script.\n\n"
            "UCX naming:\n"
            "Use UCX_<BaseAssetName> or UCX_<BaseAssetName>_01, _02, etc.\n"
            "If UCX Required is active, Safe Fix can sync old UCX names to the new base prefix.\n\n"
            "Socket naming:\n"
            "Use SOCKET_* locator/transform names for attachment points.\n\n"
            "LOD naming:\n"
            "Use SM_Asset_LOD0, SM_Asset_LOD1, SM_Asset_LOD2. Polycount should decrease by LOD level.\n\n"
            "Safe Fix policy:\n"
    ...
    def _apply_style(self):
        ...
        #M2USubtitle { color: #98a3b3; font-size: 12px; }
    ...
    def apply_preset(self, preset_name):
        settings = _deepcopy_json(PRESETS.get(preset_name, PRESETS["Marketplace Static Mesh"]))
        self.set_settings_to_ui(settings)
        self.last_settings = settings

    def reset_to_defaults(self):
        """Restore safe, user-friendly Unreal-ready defaults without modifying scene objects."""
        default_preset = "Marketplace Static Mesh"
        ...
        safe_message_box(
            ...
            "Default profile: Marketplace Static Mesh\n"
            "Prefix: SM_\n"
            "Collision: UCX optional, but validated when present\n"
            "Safe fixes: backup, rename, freeze, history cleanup and pivot fix enabled\n"
            "Reports: JSON, HTML and Unreal import script enabled"
        )
    ...
    def set_settings_to_ui(self, s):
        ...
        self.required_prefix_edit.setText(s.get("required_prefix", "SM_"))
    ...
    def get_settings_from_ui(self):
        ...
        return {
            ...
            "required_prefix": self.required_prefix_edit.text(),
    ...
    def analyze_selected_assets(self):
        if cmds is None:
            safe_message_box("M2U Pipeline Suite", "This tool must run inside Autodesk Maya.", "error")
            return
        ...
        if not results:
            safe_message_box("M2U Pipeline Suite", "No valid static mesh transform selected. Select one or more mesh transforms and analyze again.", "warning")
    ...
    def export_assets(self):
        if not self.last_results:
            ...
            if not self.last_results:
                return
        folder = QtWidgets.QFileDialog.getExistingDirectory(self, "Choose M2U Export Folder")
        if not folder:
            return
        settings = self.get_settings_from_ui()
        try:
            ...
            paths = []
            for key in ["json_report", "html_report", "unreal_import_script"]:
                if summary.get(key):
                    paths.append("{0}: {1}".format(key, summary.get(key)))
            self.report_paths_text.setPlainText("\n".join(paths) or "No report files were written.")
            safe_message_box("M2U Pipeline Suite", "Export completed. Exported: {0} | Skipped: {1} | Failed: {2}".format(summary.get("exported_assets"), summary.get("skipped_assets"), summary.get("failed_exports")))
        except Exception as exc:
    ...
    def create_box_ucx(self):
        ...
        for t in transforms:
            try:
                ...
                cmds.warning("Create UCX failed for {0}: {1}".format(t, exc))
        safe_message_box("Create Box UCX", "Created {0} simple UCX collision mesh(es). Re-run Analyze.".format(len(created)))
    ...
    def colorize_current_ucx(self):
        ...
        safe_message_box("Collision Preview", "Colorized {0} UCX mesh(es).".format(count))
    ...
    def reset_current_ucx_preview_materials(self):
        ...
        safe_message_box("Collision Preview", "Restored preview material assignment on {0} UCX mesh(es).".format(count))
    ...
    def format_result(self, r, include_actions=False, include_export=False):
        lines = []
        lines.append("=" * 80)
        lines.append("{0} | Status: {1} | M2U Score: {2}/100".format(r.get("asset_name", "UnnamedAsset"), r.get("status", "Unknown"), r.get("m2u_score", 0)))
        ...
        if matches:
            ...
            lines.append("  - No matching UCX mesh")
        if r.get("ucx_mesh_validation"):
            lines.append("UCX Mesh Validation:")
...
def show():
    """Open the M2U Pipeline Suite UI and keep a module-level reference alive."""
    global _UI_INSTANCE
    if cmds is None or QtWidgets is None:
        raise RuntimeError("M2U Pipeline Suite must run inside Autodesk Maya with PySide2/PySide6 available.")
    try:
