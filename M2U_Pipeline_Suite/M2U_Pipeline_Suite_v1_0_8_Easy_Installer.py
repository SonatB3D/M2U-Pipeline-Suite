# -*- coding: utf-8 -*-
"""
M2U Pipeline Suite v1.0.8 - Easy Maya Installer

Run inside Maya Script Editor > Python tab. The installer asks for an install
folder, copies the tool files there, and adds/updates the M2U Pipeline Suite
...
REQUIRED_FILES = [
    ...
    "README_M2U_Pipeline_Suite_v1_0_8_Regression_Restore.txt",
...
def _choose_install_folder(cmds):
    ...
    result = cmds.fileDialog2(caption="Choose M2U Pipeline Suite Install Folder", fileMode=3, startingDirectory=start_dir, okCaption="Install Here")
    if not result:
        return None
    install_dir = result[0]
    if os.path.basename(os.path.normpath(install_dir)).lower() not in ["m2u_pipeline_suite", "m2u pipeline suite"]:
        install_dir = os.path.join(install_dir, "M2U_Pipeline_Suite")
...
def _write_install_record(install_dir):
    path = os.path.join(install_dir, "m2u_pipeline_suite_install.json")
    data = {
        "tool": "M2U Pipeline Suite",
...
def run_installer():
    ...
    if not install_dir:
        print("M2U Pipeline Suite installation cancelled.")
        return
    ...
    message = (
        "M2U Pipeline Suite %s installed successfully.\n\n"
        "Install folder:\n%s\n\n"
        "Shelf:\n%s\n\n"
        "Button:\n%s\n\n"
    ...
    try:
        cmds.confirmDialog(title="M2U Pipeline Suite Installer", message=message, button=["OK"], icon="information")
    except Exception:
        pass
    print(message)
